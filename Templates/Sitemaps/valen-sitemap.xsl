...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="client-name" select="'Client Website'"/>
  ...
  <xsl:template match="/">
    <html lang="en">
      <head>
        ...
        <title><xsl:value-of select="$client-name"/> Sitemap | Valen Systems</title>
        <style>
          @font-face {
      ...
      <body>
        <div class="shell">
          ...
          <section class="intro">
            <div>
              <span class="eyebrow"><xsl:value-of select="$client-name"/></span>
              <h1>
                <xsl:choose>
              ...
              <p>
                <xsl:choose>
                  <xsl:when test="sm:sitemapindex">This index groups the site's published pages by content type so search engines and site operators can inspect the structure.</xsl:when>
                  <xsl:otherwise>This sitemap lists canonical site URLs and their latest source-controlled modification date.</xsl:otherwise>
                </xsl:choose>
              </p>
            </div>
            <div class="count">
          ...
          <footer>
            <span><xsl:value-of select="$client-name"/> sitemap presentation</span>
