...
function parseArgs(argv = process.argv.slice(2)) {
  const opts = { prefix: "seo-preview", dryRun: false, includeMedia: true, concurrency: 4 };
  for (const arg of argv) {
    if (arg === "--dry-run") opts.dryRun = true;
    else if (arg === "--skip-media") opts.includeMedia = false;
    else if (arg.startsWith("--prefix=")) opts.prefix = arg.slice("--prefix=".length).replace(/^\/+|\/+$/g, "");
...
async function run(args, { quiet = false, timeoutMs = 90000 } = {}) {
  try {
    const result = await execFileAsync(wranglerBin, args, {
      cwd: siteDir,
      timeout: timeoutMs,
...
async function collectUploads(opts) {
  const previewRoot = path.join(siteDir, opts.prefix);
  await fs.access(previewRoot);
  const previewFiles = await fg(`${opts.prefix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}/**/*`, {
    cwd: siteDir,
    onlyFiles: true,
    dot: false,
  });
  ...
  for (const rel of previewFiles) {
    uploads.push({ localFile: path.join(siteDir, rel), key: rel });
    if (rel.endsWith("/index.html")) {
      const slashKey = rel.slice(0, -"index.html".length);
...
console.log(`${opts.dryRun ? "would upload" : "uploaded"} ${uploads.length} objects to ${bucket} with prefix ${opts.prefix}`);
