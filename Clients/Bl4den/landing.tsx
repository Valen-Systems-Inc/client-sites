...
declare global {
  interface Window {
    BL4DEN_CDN_ASSET_BASE?: string;
    BL4DEN_CDN_ENTRY_URL?: string;
    BL4DEN_CDN_RELEASE?: string;
  }
}

// Production promotion guard: R2 owns the static payload; this wrapper only boots it.
const CDN_BASE = "https://bl4den.com/";
const CDN_RELEASE = "bl4den-prom-night-r2-20260613";
...
function installCdnHead(cdnDocument: Document) {
  document.title = cdnDocument.title || "Bl4den";
  document.querySelectorAll("[data-bl4den-cdn-head]").forEach((element) => element.remove());
  ...
  cdnDocument.head
    ...
    .forEach((element) => {
      ...
      clone.dataset.bl4denCdnHead = "true";
...
async function runCdnScripts(scripts: ReturnType<typeof collectScripts>) {
  for (const script of scripts) {
    await new Promise<void>((resolve, reject) => {
      ...
      element.dataset.bl4denCdnScript = "true";
      ...
      if (script.src) {
        ...
        element.onerror = () => reject(new Error(`Bl4den CDN script failed: ${element.src}`));
      } else {
...
async function loadBl4denCdnDocument() {
  const response = await fetch(CDN_ENTRY_URL, { cache: "no-store" });
  if (!response.ok) throw new Error(`Bl4den CDN entry failed: ${response.status}`);

  const html = await response.text();
  return new DOMParser().parseFromString(html, "text/html");
}

function Bl4denLandingPage() {
  ...
  useEffect(() => {
    ...
    async function bootCdnSite() {
      ...
      window.BL4DEN_CDN_ASSET_BASE = CDN_BASE;
      window.BL4DEN_CDN_ENTRY_URL = CDN_ENTRY_URL;
      window.BL4DEN_CDN_RELEASE = CDN_RELEASE;

      const cdnDocument = await loadBl4denCdnDocument();
    ...
    void bootCdnSite().catch((error) => {
      console.error("Bl4den CDN site failed to boot:", error);
      setBootError(error instanceof Error ? error.message : "Bl4den failed to boot.");
    });
  ...
  return (
    <div id="bl4den-cdn-root" ref={rootRef}>
      {bootError ? (
        <main
          style={{
            background: "#050505",
        ...
        >
          <p>Bl4den could not start. Please refresh.</p>
        </main>
      ) : null}
    </div>
  );
...
export default Bl4denLandingPage;

const container = document.getElementById("root");
if (container) {
  const root = createRoot(container);
  root.render(<Bl4denLandingPage />);
}
